...
export class GrahamScan implements IConvexHullAlgorithm {
    ...
    public findPoints(): IPoint[] {
        ...
        for (let i = 1; i < this.points.length; ++i) {
            ...
            while (i < this.points.length - 1
                && this.orientation(this.p0, this.points[i], this.points[i + 1]) === Direction.Collinear) {
                ++i;
            }
        ...
        for (let i = 3; i < m; ++i) {
            ...
            // a non-left turn
            while (this.orientation(this.findNextToTop(pointStack), pointStack[pointStack.length - 1], this.points[i])
    ...
    private orientation(p1: IPoint, p2: IPoint, p3: IPoint): number {
        const val: number =
            (p2.getY() - p1.getY()) * (p3.getX() - p2.getX()) - (p2.getX() - p1.getX()) * (p3.getY() - p2.getY());
        if (val === 0) {
    ...
    private sortPoints(): void {
        const pointsToSort: IPoint[] = this.points.slice(1);
